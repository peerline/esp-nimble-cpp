...
bool NimBLEClient::connect(const NimBLEAddress& address, uint16_t connId)
{

    m_peerAddress = address;
    m_conn_id = connId;

    ble_gattc_exchange_mtu(m_conn_id, NULL, NULL);

    // In the case of a multi-connecting device we ignore this device when
    // scanning since we are already connected to it
    NimBLEDevice::addIgnored(m_peerAddress);
...
NimBLERemoteService* NimBLEClient::getService(const NimBLEUUID& uuid)
{
    ...
    for (auto& it : m_servicesVector) {
        NIMBLE_LOGI(LOG_TAG, "<< getService: uuid: %s", uuid.toString().c_str());
